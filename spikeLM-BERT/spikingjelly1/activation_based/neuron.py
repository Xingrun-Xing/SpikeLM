...
class PSNTer(nn.Module, base.MultiStepModule):
    ...
    def forward(self, x_seq: torch.Tensor):
        # x_seq.shape = [T, B, N, C]
        h_seq = torch.addmm(self.bias, self.weight, x_seq.flatten(1))
        x_seq = h_seq.view(x_seq.shape)
        
        x_seq = x_seq / self.alpha.expand_as(x_seq)

        x_seq_hard = x_seq.clamp(-1, 1).round()
        x_seq = x_seq_hard.detach() - x_seq.detach() + x_seq
